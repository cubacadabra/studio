...
impl StudioApp {
    pub(super) fn install_local_morphs(
        &mut self,
        ...
        for (expected_id, bytes) in &packs {
            ...
            if decoded.asset.id != *expected_id {
                return Err(format!(
                    "Local morph pack ID mismatch: catalog expects {}, pack contains {}.",
                    expected_id, decoded.asset.id
                ));
            }
            renderer
                .register_morph_pack(bytes)
                .map_err(|diagnostics| Self::format_morph_diagnostics(&diagnostics))?;
            self.registered_morphs.insert(expected_id.to_string());
        }
